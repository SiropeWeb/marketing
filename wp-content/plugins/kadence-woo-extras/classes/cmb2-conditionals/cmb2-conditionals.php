...
if ( ! class_exists( 'KT_CMB2_Conditionals' ) ) {

	/**
	 * CMB2_Conditionals Plugin.
	 */
	class KT_CMB2_Conditionals {
		...
		protected function filter_field_data_to_save( $data_to_save, $field_id, $conditional_id, $attributes ) {
			if ( array_key_exists( 'data-conditional-value', $attributes ) ) {
				...
				$conditional_value         = explode(',',$attributes['data-conditional-value']);
	...
	if ( ! function_exists( 'kt_cmb2_conditionals_init' ) ) {
		/**
		 * Initialize the class.
		 */
		function kt_cmb2_conditionals_init() {
			$kt_cmb2_conditionals = new KT_CMB2_Conditionals();
		}
	}

	add_action( 'plugins_loaded', 'kt_cmb2_conditionals_init' );
