...
class KB_Form_Admin_Entries_Table_List extends WP_List_Table {
	...
	public function record_count( $status = 'publish' ) {
		...
		if ( ! empty( $search ) ) {
				...
				$query[] = $wpdb->prepare( 'AND meta_value LIKE %s', $like );
				if ( ! empty( $status ) ) {
					$query[] = $wpdb->prepare( 'AND `status` = %s', isset( $status_options[ $status ] ) ? $status : 'publish' );
				}
				$results = $wpdb->get_results( implode( ' ', $query ), ARRAY_A );
	...
	protected function extra_tablenav( $which ) {
		...
		if ( 'top' === $which ) {
			...
			if ( ! empty( $output ) ) {
				...
				// 	submit_button( __( 'Export CSV', 'kadence-blocks-pro' ), '', 'export_action', false, array( 'id' => 'export-csv-submit' ) );
				// }
			}
